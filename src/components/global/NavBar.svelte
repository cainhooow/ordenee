<script lang="ts">
	import { page } from '$app/stores';

	function currentRoute(route: String) {
		return route == $page.route.id;
	}

	// Matchers: [route1, route2]
	function routeMatch(routes: string[]) {
		console.log(routes);
		return routes.find((route) => route == $page.route.id);
	}
</script>

<nav class="navbar">
	<section class="app-title">
		<a href="/">
			<h1>Ordenne</h1>
		</a>
	</section>
	<section class="app-discover">
		<a class={routeMatch(['/clientes', '/clientes/new-client']) ? 'active' : ''} href="/clientes">
			Clientes
		</a>
		<a class={routeMatch(['/tecnicos']) ? 'active' : ''} href="/tecnicos"> Tecnicos </a>
		<a
			class={routeMatch(['/equipamentos', '/equipamentos/new-equipament']) ? 'active' : ''}
			href="/equipamentos"
		>
			Equipamentos
		</a>
		<a class={currentRoute('/ordens') ? 'active' : ''} href="/ordens"> Ordens </a>
		<a class={currentRoute('/empresa') ? 'active' : ''} href="/empresa"> Empresa </a>
	</section>
</nav>

<style lang="scss">
	.navbar {
		display: flex;
		align-items: center;
		justify-content: space-between;

		background-color: var(--primary);
		padding: 1.5rem;
	}

	.navbar .app-title {
		font-size: 1.6rem;
	}

	.navbar .app-discover {
		margin: 0.5rem;
		position: relative;
	}
	.navbar .app-discover a {
		margin: 0.5rem;
		position: relative;
		z-index: 1;
		background-color: var(--primary);
	}

	.navbar .app-discover a.active {
		background-color: var(--white);
		border-radius: 0.3rem;
		color: var(--primary);
		padding: 0.5rem;
		box-shadow: 1px 6px 15px 0.1px #983dff8f;
		transition: all 50ms ease-in-out;
	}

	.navbar .app-discover a.active:before {
		content: '';
		position: absolute;
		width: 100%;
		height: 0.1rem;
		left: 0;
		bottom: -0.5rem;
		z-index: 0;
		transition: all 200ms ease-in-out;
		z-index: 0;
	}

	.navbar .app-discover a:before {
		content: '';
		position: absolute;
		background-color: var(--thirdary);
		width: 0;
		height: 0.1rem;
		left: 0;
		bottom: -0.5rem;
		z-index: 0;
		transition: all 200ms ease-in-out;
		z-index: 0;
	}

	.navbar .app-discover a:hover::before {
		width: 100%;
	}
</style>
